...
router.post('/update_profile', async (req, res) => {
    ...
    try {
        ...
        if (at_sign) {
            const [atSignRows] = await pool.execute(
        ...
        await pool.execute(
            `UPDATE users
            SET name = ?, surname = ?, nickname = ?, at_sign = ?, gender = ?, description = ?
            WHERE id = ?`,
            [name, surname, nickname, at_sign || null, gender, description || null, userId]
        );
